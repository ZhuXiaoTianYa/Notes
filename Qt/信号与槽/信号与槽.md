...
void Widget::handlersignal1()
{
}
void Widget::handlersignal2()
{
}
...
{
    ui->setupUi(this);
    connect(this,&Widget::mysignal1,this,&Widget::handlersignal1);
    connect(this,&Widget::mysignal2,this,&Widget::handlersignal1);
    connect(this,&Widget::mysignal1,this,&Widget::handlersignal2);
    connect(this,&Widget::mysignal2,this,&Widget::handlersignal2);
...
Qt引入信号槽机制目的是为了让信号和槽之间能按多对多方式关联
其他的GUI框架往往不具备只有的特性
但实际上，随着程序开发这件事大家经验越来越多，其实在GUI开发过程中，多对多这件事其实是个伪需求
实际开发中很少会用到，绝大部分情况，一对一就够用了
新出的一些图形化开发框架，很少有再继续支持这种多对多的了
...
void Widget::handle12()
{
    this->setWindowTitle("设置窗口标题2");
    qDebug("handle2");
}
...
void Widget::on_pushButton_2_clicked()
{
    ...
    connect(ui->pushButton,&QPushButton::clicked,this,&Widget::handle12);
}
...
disconnect的使用方式和connect非常类似

disconnect的使用较少，大多数情况把信号和槽连接上就不用再管了

主动断开往往是把信号重新绑定到另一个槽函数上

如果将disconnect(ui->pushButton,&QPushButton::clicked,this,&Widget::handle1);注释，就会形成一对多



## lambda定义槽函数

定义槽函数的时候，也可以使用lambda表达式。

这是很多编程语言都支持的语法糖，本质就是一个匿名函数，主要应用在回调函数中，一次性使用



lambda表达式是一个回调函数，这个函数无法直接获取到上层作用域的变量的
...
另外也要确认捕获到的lambda内部的变量是有意义的

回调函数执行的时机是不确定的（用户什么时候点击按钮不知道的）

无论何时，用户点击了按钮，捕获到的变量都能正确使用

由于此处button是new出来的变量，生命周期跟随整个窗口（挂到对象树上，窗口关闭才会释放）

这个东西就可以在后面随时使用了。类似的this指向对象widget，这个变量在main中定义，程序退出才销毁，只要程序不退出，widget就可以用，this就可以用

如果自己创建一些对象，lambda捕获，一定要注意对象生命周期管理

lambda中可以按照值的方式捕获变量[=] 还可以按照引用[&]的方式捕获，但Qt中很少这么写，因为捕获的变量一般都是各种控件的指针，指针变量按照值传递还是引用传递都无所谓。

如果按照引用，还得更关注这个引用变量本身的生命周期

lambda语法是C++11中引入的，对于Qt5及更高版本默认是按照C++11编译的，如果使用Qt4或者更老版本，就要手动在.pro文件中添加C++11的编译选项



低耦合：一个地方出问题了，对其他地方几乎没什么影响

低内聚：写代码时，实现某个功能，围绕这个功能的代码被放到了整个项目的各个地方

高内聚：写代码的时候，某个功能点的代码都被集中放在一起了